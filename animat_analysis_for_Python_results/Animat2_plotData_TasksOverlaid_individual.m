...
for j = 1:length(options)
    for i = 1:length(taskB)
        ...
        for fig = inc
            ...
            subplot(3,2,1)
                function2use(fig, indX, rangeA, Fitness_level, gcolor, condition, [40,100])
                ylabel('Fitness (%)');
            subplot(3,2,3)
                function2use(fig, indX, rangeA, num_concepts_whole, gcolor, condition, [0,4])
                ylabel('<#concepts>');
            subplot(3,2,5)
                function2use(fig, indX, rangeB, big_phi_whole, gcolor, condition, [0,1])
                ylabel('<$\sum \varphi^{\rm Max}>$','Interpreter','latex','FontSize',12);

            subplot(3,2,2)
                function2use(fig, indX, rangeA, size_main_complex, gcolor, condition, [0,1.5])
                ylabel('#elements');
            subplot(3,2,4)
                function2use(fig, indX, rangeA, num_concepts_mip, gcolor, condition, [0,2])
                ylabel('<#concepts>');
            subplot(3,2,6)
                function2use(fig, indX, rangeB, big_phi_mip, gcolor, condition, [0,0.4])
            ...
            numMot = 2;
            numSen = 2;
            J_tempfile = strcat(path, int2str(evaluatedTrials(fig)),'_', int2str(g), '_EdgeList.txt')
